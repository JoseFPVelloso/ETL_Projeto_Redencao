...
class App(tk.Tk):
    def __init__(self):
        ...
        self.final_excel_path = None
        self.final_txt_path = None
        self.final_quadras_path = None
    ...
    def create_widgets(self):
        ...
        self.btn_run_quadras = ttk.Button(
            quadras_frame, text="Gerar Relatório por Quadras",
            state="disabled", command=self.run_quadras_generator
        )
        ...
        self.btn_open_excel = ttk.Button(
            results_frame, text="Abrir Diário (.xlsx)",
            state="disabled", command=lambda: self.open_path(self.final_excel_path)
        )
        self.btn_open_excel.pack(side="left", padx=5, pady=5)

        self.btn_open_txt = ttk.Button(
            results_frame, text="Abrir Análise (.txt)",
            state="disabled", command=lambda: self.open_path(self.final_txt_path)
        )
        self.btn_open_txt.pack(side="left", padx=5, pady=5)

        self.btn_open_quadras = ttk.Button(
            results_frame, text="Abrir Quadras (.xlsx)",
            state="disabled", command=lambda: self.open_path(self.final_quadras_path)
        )
    ...
    def select_raw_file(self):
        ...
        if filepath:
            ...
            # Reset passos posteriores ao trocar o arquivo fonte
    ...
    def open_path(self, path):
        if not path or not os.path.exists(str(path)):
            messagebox.showwarning("Aviso", "Caminho não encontrado ou ainda não gerado.")
            return
        try:
    ...
    def run_parser_thread(self, filepath):
        ...
        try:
            ...
            if processed_path:
                self.msg_queue.put(("DONE_PARSER", processed_path))
            else:
                raise Exception("Falha no parser.")
        except Exception as e:
            self.msg_queue.put(("ERROR", f"Erro no Parser: {e}"))
    ...
    def run_report_generator(self):
        ...
        # FIX: validação de datas com mensagem clara incluindo a restrição de mínimo 2 dias
        try:
            ...
            if end_date <= start_date:
                raise ValueError(
                    ...
                    "O relatório precisa de pelo menos 2 dias no intervalo."
                )
        ...
        threading.Thread(
            ...
            args=(filepath, start_date, end_date),
            daemon=True
        ).start()
    ...
    def run_report_thread(self, filepath, start_date, end_date):
        def log_callback(message):
            self.msg_queue.put(message)
        try:
            excel_path, txt_path = logic_report.execute_report_generator(
                filepath, start_date, end_date, log_callback
            )
            if excel_path and txt_path:
                # FIX: enviar como tupla nomeada para desempacotamento seguro no check_queue
                self.msg_queue.put(("DONE_REPORT", excel_path, txt_path))
            else:
                raise Exception("Falha no gerador.")
        except Exception as e:
            self.msg_queue.put(("ERROR", f"Erro no Report: {e}"))
    ...
    def run_quadras_generator(self):
        ...
        if not input_excel or not os.path.exists(input_excel):
            messagebox.showerror(
                "Erro",
                "Arquivo do Relatório Diário não encontrado. Rode o Passo 2 primeiro."
            )
            self.set_ui_state("normal")
            return

        threading.Thread(
            target=self.run_quadras_thread,
            args=(input_excel,),
            daemon=True
        ).start()
        ...
        try:
            while True:
                ...
                if isinstance(msg, tuple):
                    ...
                    if msg_type == "DONE_PARSER":
                        self.log("✓ Parser concluído.")
                        self.processed_file_path.set(msg[1])
                        self.set_ui_state("normal")

                    elif msg_type == "DONE_REPORT":
                        self.log("✓ Relatório Diário concluído.")
                        # FIX: desempacotamento explícito e seguro dos 3 campos
                        self.final_excel_path = msg[1]
                        self.final_txt_path = msg[2]
                        ...
                        messagebox.showinfo(
                            "Sucesso",
                            "Passo 2 Concluído! Você já pode abrir os arquivos ou prosseguir para o Passo 3 (Quadras)."
                        )

                    elif msg_type == "DONE_QUADRAS":
                        self.log("✓ Relatório Quadras concluído.")
                        self.final_quadras_path = msg[1]
                        self.btn_open_quadras.config(state="normal")
                        self.set_ui_state("normal")
                        self.btn_run_quadras.config(state="normal")
                        messagebox.showinfo("Sucesso", "Relatório de Quadras gerado com sucesso!")

                    elif msg_type == "DONE_INSTALL":
                        self.log(msg[1])
                        self.set_ui_state("normal")
