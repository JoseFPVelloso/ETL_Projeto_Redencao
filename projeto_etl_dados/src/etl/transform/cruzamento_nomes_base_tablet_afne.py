...
def cruzar_planilhas():
    ...
    if 'NomePaciente' not in df_tablet.columns:
        print(f"ERRO: Coluna 'NomePaciente' não encontrada em Base Tablet.xlsx")
        print(f"Colunas disponíveis: {list(df_tablet.columns)}")
        return
    ...
    df_tablet['NomePaciente_Normalizado'] = df_tablet['NomePaciente'].apply(normalizar_texto)
    ...
    print("\nExemplos de nomes normalizados da base tablet:")
    for nome in df_tablet['NomePaciente_Normalizado'].head(10):
        print(f"  - {nome}")
    
    # Realiza o cruzamento pelos nomes normalizados
    df_cruzado = df_tablet[df_tablet['NomePaciente_Normalizado'].isin(df_abordados['Nomes_Normalizados'])]
    
    # Remove as colunas normalizadas do resultado final
    df_cruzado = df_cruzado.drop(columns=['NomePaciente_Normalizado'])
    
    # Salva o resultado
    output_path = os.path.join(processed_dir, 'cruzamento_nomes_base_tablet.xlsx')
    ...
    if len(df_cruzado) > 0:
        ...
        for nome in df_cruzado['NomePaciente'].head(10):
            print(f"  - {nome}")
        
        # Mostra também os nomes correspondentes da planilha de abordados
        nomes_cruzados_normalizados = df_cruzado['NomePaciente'].apply(normalizar_texto).unique()
        ...
        encontrar_correspondencias_parciais(df_abordados, df_tablet)

def encontrar_correspondencias_parciais(df_abordados, df_tablet):
    """
    ...
    nomes_tablet = set(df_tablet['NomePaciente_Normalizado'].dropna())
    
    correspondencias = []
    
    for nome_abordado in nomes_abordados:
        for nome_tablet in nomes_tablet:
            # Verifica se um nome contém o outro (correspondência parcial)
            if nome_abordado in nome_tablet or nome_tablet in nome_abordado:
                ...
                nome_tablet_original = df_tablet[df_tablet['NomePaciente_Normalizado'] == nome_tablet]['NomePaciente'].iloc[0]
                
                correspondencias.append({
                    'Nome_Abordado': nome_abordado_original,
                    'Nome_Tablet': nome_tablet_original,
                    'Normalizado_Abordado': nome_abordado,
                    'Normalizado_Tablet': nome_tablet
                })
    ...
    if correspondencias:
        ...
        for i, corresp in enumerate(correspondencias[:20]):  # Mostra no máximo 20
            print(f"  {i+1}. '{corresp['Nome_Abordado']}' -> '{corresp['Nome_Tablet']}'")
        ...
        if salvar.lower() == 's':
            ...
            output_corresp_path = os.path.join(
                ...
                'data', 'processed', 'correspondencias_parciais.xlsx'
            )
